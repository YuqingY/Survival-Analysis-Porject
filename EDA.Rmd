...
```
##Introduction
  Accoring to a New York Time article "Tough Question to Answer, Tough Answer to Hear," prognosticating is one of the most challenging tasks doctors face. However, despite the chanllenging and imperfect nature of prognosis, it is very important for a pateint to know how much time he/she has left, given that there is often much to plan for and accomplish before a progressive illness robs patients of their physical or mental abilities. For patients carrying human immunodeficiency virus type 1 (HIV 1), it is importnat for them to know how much time do they have left until the development of the auired immunodeficiency syndrome (AIDS) or death, and more importantly, how to prolong their life before death. 
  To answer this question, we use the data from a double-blind, placebo-controlled trial that compared the three-drug regimen of indinavir (IDV), open label zidovudine (ZDV) or stavudine (d4T) and lamivudine (3TC) with the two-drug regimen of zidovudine or stavudine and lamivudine in HIV-infected patients (Hammer et al., 1997). Besides having the treatment method, this dataset also has a lot of other information about the patients, including their age, sex, race, IV drug use history, hemophiliac, Karnofsky Performance Scale, as well as baseline CD4 count. Using the data, we want to know what variables contribute to a longer time to event (development of AIDS or death) and use those variables to create a model that predicting the time to the development of the AIDS or death for HIV-1 patients.
  We choose to use a cox proportional harzard model to predict the time to event. We use AIC method for model selection, and assume proportional hazard.
...
###1.1.1 Definition and derivation of AIC & BIC
    ...
    The derivation of AIC and BIC is based on 
...
###1.4 Interpretation and conclusion
...
```{r}
coxph(Surv(time,censor == 1) ~ tx * ivdrug * karnof * cd4 * age,data = AIDS)
```

=======
>>>>>>> f820fa903493985f83c6767c96b9c7561af492fc
