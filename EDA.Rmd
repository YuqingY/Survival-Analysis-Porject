...
##Introduction
  ...
  2. If yes, how does the three-drug regimen compare to the two-drug regimen in affecting the progression of AIDS and death while controlling those significant variables?  
  To answer those questions, we tried to reject the null hypothesis that none of the other variables are significant in affecting the progression of AIDS. We choose to build a Cox Proportional Hazard model to evaluate how each variable in the dataset affect the time to event(the development of AIDS or death). We use the AIC and BIC methods to assess the significance level of the variables for model building. We assume proportional hazard, which is also tested in the project, and use the final Cox Proportional Hazard model to assess the effect of adding IDV on slowing down the progression of AIDS.  
...
###Conclusion
The result of survival analysis suggests that IDV treatment decreases the hazard of development of AIDS or dying by a factor of more than half (0.485) compared to treatments without IDV, holding all other conditons equal. Since the p-values of all the coefficients are very small, we can conclude that variables treatment, Karnofsky Performance Scale, IV drug consumption history, baseline CD4 count, and age are all statistically significant in affecting the hazard of developing AIDS or death. From the interpretation of coefficients above, we can see that among all the variables in the model, treatment that includes IDV decreases the hazard of development of AIDS or dying the most. Therefore, IDV is an effective treatment on decreasing hazard of development of AIDS or dying.

##Reference
1. Class Notes on AIC/BIC and PH assumption
...
3. I will use r documentation(linked below) as well as the UCLA survival to learn about the cox.zph function.
